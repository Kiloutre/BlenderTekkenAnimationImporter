...
from .TekkenAnimHelper import TekkenAnimation
...
class Animation0xC8():
    def __init__(self, binary_data):
        ...
        header_size = TekkenAnimation._getHeaderSizeFromArgs_(bone_count)
        struct_packer = struct.Struct("< 69f")
        #animation_frame_size = struct_packer.size
        animation_frame_size = TekkenAnimation._getFramesizeFromArgs_(bone_count)
        print("%x %x %d" % (bone_count, header_size, animation_frame_size))
        ...
        for frame_index in range(self.AnimationHeader0xC8.properties['AnimationLength']):
            ...
            print("%x" % animation_frame_data_offset)

            self.check_if_can_read_frame_from_binary_data(animation_frame_data_offset, struct_packer.size,
