...
class AnimationHeader0xC8():
    animation_header_properties_ordered_dict = OrderedDict([('AnimationSignatureByte1', 0xC8),
    ...
    def __initialize_animation_header(self, header_tuple):
        header_properties_list = list(self.animation_header_properties_ordered_dict.items())

        for index in range(len(header_properties_list)):
            key = header_properties_list[index][0]
            value = header_properties_list[index][1]
            self.animation_header_properties_ordered_dict[key] = header_tuple[index]
